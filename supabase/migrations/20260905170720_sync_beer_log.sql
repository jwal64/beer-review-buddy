-- Correction, hand-added ahead of the generated block below: PR #13
-- (20260905052233_sync_beer_log.sql) logged this Tarrytown, Sep 2026 pour
-- under the wrong beer name, "Amstel" — it was actually Amstel Light. That
-- migration already ran, so the wrong row is sitting in the database under a
-- name data.js no longer has any way to reach (the generated SQL below only
-- inserts-if-missing and updates on a name match, and never deletes, so a
-- rename needs an explicit one-off delete rather than the normal add-and-
-- update). Everything else PR #13 touched — the brewery, the Tarrytown
-- location, the "Amstel Light" row itself — is corrected in place by the
-- generated block that follows, so only the two rows keyed on the old name
-- need removing here.
