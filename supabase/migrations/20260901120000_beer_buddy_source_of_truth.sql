...
-- tools/export-supabase-seed.mjs in jwal64/JWAL-BEER-REVIEW. It replaces the
-- earlier partial import rather than adding to it.


delete from public.app_meta;
delete from public.untappd_averages;
delete from public.want_to_try;
...
('Weihenstephaner','Freising, Bavaria','Germany','DE','de',null,48.3953,11.7291),
...
insert into public.beers (seq,name,brewery,style,origin_cc,abv,method,city,region,country,cc,rating,is_new,drank_on,logo) values
(1,'Grolsch','Grolsch','Pilsner','NL',5,'Bottle','Hengelo','Overijssel','Netherlands','NL',3.5,false,'2026-01-01',null),
...
('Affligem Tripel',array['affligembeer.be']),
...
(1,'Paulaner Hefe','Wheat Beer','DE',5.5,'Munich, Bavaria',3.87,'Bottle',array['Paulaner Hefe-Weißbier']),
...
('untappd_last_refreshed','2026-05-05'),
('untappd_refresh_interval_days','14');

-- ── What the site cannot render without ───────────────────────
-- Applied after the seed, because these columns were nullable while the
-- earlier partial import was in the table and are only complete now.
--
...
alter table public.breweries alter column lang set not null;

alter table public.beers alter column origin_cc set not null;
alter table public.beers alter column abv set not null;
alter table public.beers alter column method set not null;
alter table public.beers alter column city set not null;
alter table public.beers alter column region set not null;
alter table public.beers alter column country set not null;
alter table public.beers alter column cc set not null;
