...
check('numeric columns arriving as strings are not differences', () => {
  ...
  const strung = rows => rows.map(row => Object.fromEntries(
    Object.entries(row).map(([k, v]) => [k, typeof v === 'number' ? String(v.toFixed(k === 'rating' || k === 'abv' ? 2 : 4)) : v])));
...
check('an empty database is every row missing', () => {
  ...
  eq(kinds(r).every(k => k === 'missing'), true, 'all missing');
});
