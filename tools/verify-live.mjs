...
function same(exp, act) {
  ...
  if (Array.isArray(exp)) {
    return Array.isArray(act) && act.length === exp.length
      && exp.every((v, i) => same(v, act[i]));
  }
...
export function compareAll(expected, actual) {
  ...
  for (const table of TABLES) {
    ...
    for (const row of act) {
      ...
      if (actBy.has(k)) {
        errors.push({
          kind: 'duplicate', table, row: label(table, row),
    ...
    for (const [k, row] of actBy) {
      ...
      if (stranded) {
        errors.push({
          kind: 'orphan', table, row: label(table, row),
          detail: table === 'beers'
...
function connection() {
  ...
  const key = process.env.SUPABASE_PUBLISHABLE_KEY || env.SUPABASE_PUBLISHABLE_KEY
    || env.VITE_SUPABASE_PUBLISHABLE_KEY || liveKey;
  if (!url || !key) throw new Error('no Supabase URL/key found in env, .env or live-data.js');

  const warn = liveUrl && liveUrl.replace(/\/$/, '') !== String(url).replace(/\/$/, '')
  ...
  for (const table of TABLES) {
    ...
    for (let offset = 0; ; offset += PAGE) {
      const res = await fetch(
        `${url}/rest/v1/${table}?select=*&limit=${PAGE}&offset=${offset}`, { headers });
      if (!res.ok) {
...
function render({ errors, notices }, expected, actual) {
  ...
  lines.push('');
  lines.push(`  data.js  ${expected.beers.length} reviews · ${expected.breweries.length} breweries · ${expected.brand_domains.length} brands`);
  lines.push(`  database ${actual.beers.length} reviews · ${actual.breweries.length} breweries · ${actual.brand_domains.length} brands`);
  lines.push('');
...
if (isMain) {
  ...
  let result, actual, unreachable = null;
